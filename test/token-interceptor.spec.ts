...
makeSuite("BNFT: Token Interceptor", (testEnv: TestEnv) => {
  ...
  it("Single Interceptor add and delete some tokens", async () => {
    ...
    await waitForTx(await _mockBNFTMinter1.addTokenBurnInterceptor(_user0TokenId1, _mockTokenInterceptor1.address));
    const checkInterceptors1 = await bBAYC.getTokenBurnInterceptors(_mockBNFTMinter1.address, _user0TokenId1);
    expect(checkInterceptors1.length).eq(1);
    expect(checkInterceptors1[0]).eq(_mockTokenInterceptor1.address);

    await waitForTx(await _mockBNFTMinter1.deleteTokenBurnInterceptor(_user0TokenId1, _mockTokenInterceptor1.address));
    const checkInterceptors2 = await bBAYC.getTokenBurnInterceptors(_mockBNFTMinter1.address, _user0TokenId1);
  ...
  it("Multiple Interceptors add and delete some tokens", async () => {
    ...
    await waitForTx(await _mockBNFTMinter1.addTokenBurnInterceptor(_user0TokenId1, _mockTokenInterceptor2.address));
    const checkInterceptors2 = await bBAYC.getTokenBurnInterceptors(_mockBNFTMinter1.address, _user0TokenId1);
    expect(checkInterceptors2.length).eq(2);
    expect(checkInterceptors2[1]).eq(_mockTokenInterceptor2.address);

    await waitForTx(await _mockBNFTMinter1.deleteTokenBurnInterceptor(_user0TokenId1, _mockTokenInterceptor1.address));
    const checkInterceptors3 = await bBAYC.getTokenBurnInterceptors(_mockBNFTMinter1.address, _user0TokenId1);
    expect(checkInterceptors3.length).eq(1);
    expect(checkInterceptors3[0]).eq(_mockTokenInterceptor2.address);

    await waitForTx(await _mockBNFTMinter1.deleteTokenBurnInterceptor(_user0TokenId1, _mockTokenInterceptor2.address));
    const checkInterceptors4 = await bBAYC.getTokenBurnInterceptors(_mockBNFTMinter1.address, _user0TokenId1);
    expect(checkInterceptors4.length).eq(0);
  });

  it("Remove All Interceptors when burn", async () => {
  ...
  it("Single interceptor has been called when burn", async () => {
    const { users, bBAYC } = testEnv;
    ...
    await waitForTx(await _mockBNFTMinter2.addTokenBurnInterceptor(_user0TokenId1, _mockTokenInterceptor2.address));
    ...
    const checkBeforeBurnCalled1 = await _mockTokenInterceptor1.isBeforeTokenBurnCalled();
    expect(checkBeforeBurnCalled1).eq(true);

    const checkAfterBurnCalled1 = await _mockTokenInterceptor1.isAfterTokenBurnCalled();
    expect(checkAfterBurnCalled1).eq(true);

    // minter 2, interceptor 2 should not be called
    const checkBeforeBurnCalled2 = await _mockTokenInterceptor2.isBeforeTokenBurnCalled();
    expect(checkBeforeBurnCalled2).eq(false);

    const checkAfterBurnCalled2 = await _mockTokenInterceptor2.isAfterTokenBurnCalled();
    expect(checkAfterBurnCalled2).eq(false);
  });

  it("Multiple interceptors has been called when burn", async () => {
    const { users, bBAYC } = testEnv;
    ...
    const checkBurnCalled1 = await _mockTokenInterceptor1.isBeforeTokenBurnCalled();
    expect(checkBurnCalled1).eq(true);

    // minter 1, interceptor 2 will be called
    const checkBurnCalled2 = await _mockTokenInterceptor2.isBeforeTokenBurnCalled();
